...
u8 CheckArenaLimits(void)
{
	...
	{
		if (ArenaLimitTable[x].ChapterID == gChapterData.chapterIndex)
		{
			if (ArenaLimitTable[x].TurnLimit)
			{	
				if (gChapterData.turnNumber > ArenaLimitTable[x].TurnLimit)
				{
...
void KillUnitIfNoHealth(Unit* unit)
{
...
u8 DidUnitDie(BattleUnit* bUnit)
{
	u8 *ArenaVictoryByte = &gArenaData + 0xA;
